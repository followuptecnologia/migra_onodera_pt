...
from apps.functions import leTraducaoId, criaTraducaoId, atualizaTraducaoId, balance_sku, recalcula_saldo

from apps import parametros
from apps.migrate.onodera.save.models import (
    CorporateGroupUnity, Partner, AccessUser,
    Sku,
    Schedule, ScheduleStatus, ScheduleSku, ScheduleSkuResource, ScheduleStatusDetail, Resource, SalesOrderSku,
)
            ...
            if response_agendamento_json["colunaId"] is not None and response_agendamento_json["tratamentoId"] != "1":
                ...
                if not schedule:
                    ...
                    if sku_servico is not None:
                        sales_order_sku = None
                        list_sales_order_sku = SalesOrderSku.objects.filter(
                            sku_id=sku_servico.id,
                            sales_order__customer_partner=partner_customer.id,
                            sales_order__order_status__in=['saleCompleted', 'partialSale', 'totalPendingValue'],
                            quantity_sold=1
                        )
                        for sale_sku in list_sales_order_sku:
                            balance = balance_sku(sale_sku.id)
                            quantity_available = (
                                balance["quantity_available_schedule"] if balance else 0
                            )
                            if quantity_available > 0:
                                sales_order_sku = sale_sku
                                break
                    ...
                    if not recurso_profissional:
                        ...
                        schedule_sku_professional.sales_order_sku_id = sales_order_sku.id
                        schedule_sku_professional.save()

                    recalcula_saldo(sales_order_sku.sales_order)
